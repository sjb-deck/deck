...
describe('CartPopupModal', () => {
  ...
  it('displays the dropdown when there are multiple expiry dates', async () => {
    ...
    expect(
      within(screen.getByRole('combobox', { name: 'Expiry Date' })).getByText(
  ...
  it('updates selectedExpiry when a date is clicked', async () => {
    ...
    expect(
      ...
      ).getByText(mockItemWithDates.expiry_dates[1].expiry_date),
    ).toBeInTheDocument();
  });
  ...
  it('display calendar when New is clicked and update selected date', async () => {
    ...
    const expiryDateCombobox = within(
      screen.getByRole('combobox', { name: 'Expiry Date' }),
    );
    expect(
      expiryDateCombobox.getByText(dayjs().format('YYYY-MM-DD')),
    ).toBeInTheDocument();
    expect(expiryDateCombobox.getByText('(New)')).toBeInTheDocument();
  });
