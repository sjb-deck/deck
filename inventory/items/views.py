...
def check_for_alerts(request):
    ...
    # Get item expiries that are expiring within 1 month or expired
    ...
    expiring_items = ItemExpiry.objects.filter(Q(expiry_date__lte=one_month_later) & Q(expiry_date__gte=current_date))

    response_data = {
        'low_quantity_items': [{'name': item.name, 'total_quantity': item.total_quantity} for item in low_quantity_items],
        'expired_items' : [{'name': expiry.item.name, 'expiry_date': expiry.expiry_date, 'quantity': expiry.quantity, 'status': 'expired'} for expiry in expired_items],
        'expiring_items': [{'name': expiry.item.name, 'expiry_date': expiry.expiry_date, 'quantity': expiry.quantity, 'status': 'expiring'} for expiry in expiring_items],
    }
