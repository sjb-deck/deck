...
class TestApiGetNewKitRecipeViews(TestCase):
    ...
    def test_get_recipe(self):
        response = self.client.get(reverse("get_new_kit_recipe", args=[self.blueprint_id]), None, format="json")
        ...
        self.assertEqual(response.data[0]["item_options"][0]["item_expiry_id"], self.itemExpiry1.id)
        self.assertEqual(response.data[0]["item_options"][0]["quantity"], 50)
        self.assertEqual(response.data[0]["item_options"][1]["item_expiry_id"], self.itemExpiry2.id)
        ...
        self.assertEqual(response.data[1]["item_options"][0]["item_expiry_id"], self.itemExpiry_no_expiry.id)
        self.assertEqual(response.data[1]["item_options"][0]["quantity"], 50)

    def test_get_recipe_with_invalid_blueprint(self):
        response = self.client.get(reverse("get_new_kit_recipe", args=[0]), None, format="json")
